...
function discoverCallers(): { found: string[]; unclassified: string[]; shell: string[] } {
  ...
  const classify = (abs: string): void => {
    ...
    if (argvTails(src).length > 0) found.push(toPosix(abs));
    else if (mentionsProbe(src)) unclassified.push(toPosix(abs));
  };
