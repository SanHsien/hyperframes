...
describe("resolveProxy", () => {
  ...
  it("transcodes once on a cache miss and caches via temp+rename", async () => {
    ...
    expect(cacheDirEntries).toEqual([basename(expectedCachePath)]);
  });
  ...
  it("proxies an external target reached through an in-project symlink", async (context) => {
    const { spawn, calls } = createSpawnSpy();
    const { resolveProxy, getProxyCachePath } = await loadModule(spawn, FFMPEG_PATH);
    const projectDir = tmpProject();
    const outsideDir = tmpProject();
    ...
    try {
      ...
      if (
        ...
        typeof error === "object" &&
        "code" in error &&
        ["EPERM", "EACCES", "ENOSYS"].includes(String((error as any).code))
      ) {
