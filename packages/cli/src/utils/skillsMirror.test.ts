...
const canCreateSymlinks = (() => {
  try {
    const tmp = mkdtempSync(join(tmpdir(), "hf-symlink-check-"));
...
describe("mirrorGlobalSkills", () => {
  it.skipIf(!canCreateSymlinks).each([
    [
      "absolute",
  ...
  it("honors XDG_CONFIG_HOME for config-based agents", () => {
    ...
    const { mirrored } = mirrorGlobalSkills({
      skills: ["hyperframes"],
      home,
      platform: canCreateSymlinks ? "linux" : "win32",
  ...
  it.skipIf(!canCreateSymlinks)("is idempotent and refreshes stale entries", () => {
    const home = makeHome();
    seedStore(home, ["hyperframes"]);
    installMarker(home, ".cursor");
  ...
  it("is idempotent and refreshes stale entries on Windows", () => {
    const home = makeHome();
    seedStore(home, ["hyperframes"]);
    installMarker(home, ".cursor");
